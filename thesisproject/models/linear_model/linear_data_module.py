...
from .image_stack_dataset import ImageStackDataset
...
class LinearDataModule(pl.LightningDataModule):
    ...
    def __init__(
        ...
        # remove samples with less than n_visits
        self.subjects_df = self.subjects_df.groupby("subject_id_and_knee").filter(lambda x: len(x) >= n_visits)
    ...
    def collate_test_fn(self, batch):
        imagestack, label = batch[0].image, batch[0].label
        return [imagestack, label]
    ...
    def setup(self, stage: str):
        ...
        # train data
        self.train_dataset = ImageStackDataset(self.data_dir, train_df, image_transform=SquarePad(), n_visits=self.n_visits)

        self.train_slice_loader = SliceSeriesLoader(self.train_dataset, slices_per_epoch=self.train_slices_per_epoch)

        # val data
        val_dataset = ImageStackDataset(self.data_dir, val_df, image_transform=SquarePad(), n_visits=self.n_visits)

        self.val_slice_loader = SliceSeriesLoader(val_dataset, slices_per_epoch=self.train_slices_per_epoch, elastic_deform=False)

        # test data
        self.test_dataset = ImageStackDataset(self.data_dir, test_df, image_transform=SquarePad(), n_visits=self.n_visits)

        #self.test_slice_loader = SliceSeriesLoader(self.test_dataset, slices_per_epoch=self.test_slices_per_epoch, elastic_deform=False)
    ...
    def test_dataloader(self):
        return DataLoader(self.test_dataset, batch_size=1, num_workers=1, pin_memory=True, collate_fn=lambda imagestack: imagestack)