...
class LitFullLSTM(pl.LightningModule):
    def __init__(self, unet, encoder, lstm, lr=5e-5, weight_decay=1e-5):
        super().__init__()
        self.unet = unet
    ...
    def _predict_view(self, image_volumes, timedeltas):
        with torch.no_grad():
            view_predictions = []
            for image_slice_batch in torch.split(image_volumes, 8, dim=1):
                ...
                # TODO create series
                encoded_series = torch.stack(encoded_series).permute(1, 0, 2) #8 x K x L

                timedeltas_batch = timedeltas.repeat(encoded_series.shape[0]).reshape(encoded_series.shape[0], -1).unsqueeze(-1)
                encoded_inputs_dt = torch.cat([timedeltas_batch, encoded_series], dim=2)

                encoded_batch = encoded_inputs_dt

                outputs = self.lstm(encoded_batch)
                prediction = softmax(outputs, dim=1).squeeze()

                view_predictions.append(prediction[:,1].detach().cpu())
            return torch.cat(view_predictions)

    def _predict_volumes(self, image_volumes, timedeltas):
        # First view
        prediction_view = []
        prediction_view.append(self._predict_view(image_volumes, timedeltas))

        # Second view
        rot_img = torch.stack([image_volume.permute(1, 0, 2) for image_volume in image_volumes])
        prediction_view.append(self._predict_view(rot_img, timedeltas))
    ...
    def predict(self, image_volumes, timedeltas):
        """
        image_volumes is series of image volumes: K x H x W x D
        timedeltas is list of length K
        """
        volume_prediction = self._predict_volumes(image_volumes.squeeze(1), timedeltas)
