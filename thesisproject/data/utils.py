...
def get_slice_loaders(path, volume_transform=None, label_transform=None):
    train_data = ImagePairDataset(path + "train", image_transform=volume_transform, label_transform=label_transform)
    #train_queue = ImageQueue(train_data)
    train_loader = SliceLoader(train_data, slices_per_epoch=2000)

    val_data = ImagePairDataset(path + "val", image_transform=volume_transform, label_transform=label_transform)
